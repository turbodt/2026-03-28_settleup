...
#define SPM_MATRIX_HEADER(S_T, T_SFX, F_SFX) \
 \
 \
typedef struct SpmMatrix##T_SFX SpmMatrix##T_SFX; \
 \
 \
SpmMatrix##T_SFX * spm_matrix_##F_SFX##_make(size_t row_count, size_t col_count); \
SpmMatrix##T_SFX * spm_matrix_##F_SFX##_clone(SpmMatrix##T_SFX const *src); \
void spm_matrix_##F_SFX##_destroy(SpmMatrix##T_SFX *m); \
 \
 \
size_t spm_matrix_##F_SFX##_get_col_count(SpmMatrix##T_SFX const *m); \
size_t spm_matrix_##F_SFX##_get_row_count(SpmMatrix##T_SFX const *m); \
S_T spm_matrix_##F_SFX##_get(SpmMatrix##T_SFX const *m, size_t row, size_t col); \
SpmErr spm_matrix_##F_SFX##_set( \
    SpmMatrix##T_SFX *m, \
    size_t row, \
    size_t col, \
    S_T value \
); \
void spm_matrix_##F_SFX##_transpose(SpmMatrix##T_SFX *m); \
SpmMatrix##T_SFX * spm_matrix_##F_SFX##_prod(SpmMatrix##T_SFX const *left, SpmMatrix##T_SFX const *right); \
...
#define SPM_MATRIX_IMPLEMENTATION(S_T, ZERO, IS_ZERO, SUM, PROD, T_SFX, F_SFX) \
 \
 \
...
typedef struct { \
 ...
 \
 \
SPM_LIST_HEADER(SpmMatrix##T_SFX##Entry, T_SFX##Entry, F_SFX##_entry); \
SPM_LIST_IMPLEMENTATION(SpmMatrix##T_SFX##Entry, T_SFX##Entry, F_SFX##_entry); \
 \
 \
struct SpmMatrix##T_SFX { \
    size_t col_count; \
    size_t row_count; \
 \
    SpmList##T_SFX##Entry entries; \
 ...
 \
 \
static SpmErr matrix_prod_tn(SpmMatrix##T_SFX const *, SpmMatrix##T_SFX const *, SpmMatrix##T_SFX *); \
static SpmErr matrix_prod_nn(SpmMatrix##T_SFX const *, SpmMatrix##T_SFX const *, SpmMatrix##T_SFX *); \
static SpmErr matrix_prod_nt(SpmMatrix##T_SFX const *, SpmMatrix##T_SFX const *, SpmMatrix##T_SFX *); \
static SpmErr matrix_value_remove(SpmMatrix##T_SFX *, size_t row, size_t col); \
...
static SpmErr matrix_entry_insert_at( \
    SpmMatrix##T_SFX *, \
    size_t entry_index, \
    SpmMatrix##T_SFX##Entry entry \
); \
static void matrix_toggle_transpose_bool(SpmMatrix##T_SFX *); \
static void matrix_transpose_values(SpmMatrix##T_SFX *); \
static void matrix_rebuild_col_indexes(SpmMatrix##T_SFX *); \
...
static size_t entry_index_get_at(SpmMatrix##T_SFX const *, size_t row, size_t col); \
...
static void entry_swap(SpmMatrix##T_SFX##Entry *, SpmMatrix##T_SFX##Entry *); \
static void list_size_swap(SpmListSize *, SpmListSize *); \
static void list_entry_swap(SpmList##T_SFX##Entry *, SpmList##T_SFX##Entry *); \
...
SpmMatrix##T_SFX * spm_matrix_##F_SFX##_make(size_t row_count, size_t col_count) { \
    int err; \
 \
    SpmMatrix##T_SFX *m = malloc(sizeof(SpmMatrix##T_SFX)); \
    if (!m) { \
 ...
 \
    ...
    m->bools = 0; \
    err = spm_list_##F_SFX##_entry_init(&m->entries, max_dim_count); \
    if (err) { \
...
MatrixMakeRowIndexesAllocFailed: \
    spm_list_##F_SFX##_entry_clear(&m->entries); \
...
SpmMatrix##T_SFX * spm_matrix_##F_SFX##_clone(SpmMatrix##T_SFX const *src) { \
    SpmErr err; \
    SpmMatrix##T_SFX *dst = malloc(sizeof(SpmMatrix##T_SFX)); \
    if (!dst) { \
        return NULL; \
    } \
 \
    ...
    dst->bools = src->bools; \
 \
    spm_list_##F_SFX##_entry_init(&dst->entries, 0); \
    spm_list_size_init(&dst->row_indexes, 0); \
    spm_list_size_init(&dst->col_indexes, 0); \
 \
    err = spm_list_##F_SFX##_entry_cpy(&dst->entries, &src->entries); \
    if (err != SPM_ERR__OK) { \
...
MatrixCloneListCopyFailed: \
    spm_matrix_##F_SFX##_destroy(dst); \
    return NULL; \
}; \
 \
 \
void spm_matrix_##F_SFX##_destroy(SpmMatrix##T_SFX *m) { \
 ...
 \
 \
inline size_t spm_matrix_##F_SFX##_get_col_count(SpmMatrix##T_SFX const *m) { \
    return __SPM_MATRIX_IS_TRANSPOSED(m) ? m->row_count : m->col_count; \
}; \
 \
 \
inline size_t spm_matrix_##F_SFX##_get_row_count(SpmMatrix##T_SFX const *m) { \
...
S_T spm_matrix_##F_SFX##_get(SpmMatrix##T_SFX const *m, size_t row, size_t col) { \
 ...
 \
    SpmMatrix##T_SFX##Entry const *entry = entry_get_at((SpmMatrix##T_SFX *)m, row, col); \
    if (!entry) { \
        return ZERO; \
    } \
    return entry->value; \
...
SpmErr spm_matrix_##F_SFX##_set( \
 ...
 \
    SpmMatrix##T_SFX##Entry * entry = entry_get_at(m, row, col); \
    int new_value_is_zero = IS_ZERO(value); \
 \
 ...
 \
 \
inline void spm_matrix_##F_SFX##_transpose(SpmMatrix##T_SFX *m) { \
    matrix_toggle_transpose_bool(m); \
}; \
 \
 \
SpmMatrix##T_SFX * spm_matrix_##F_SFX##_prod(SpmMatrix##T_SFX const *A, SpmMatrix##T_SFX const *B) { \
    SpmErr err; \
    size_t const common_count = spm_matrix_##F_SFX##_get_col_count(A); \
    if (common_count != spm_matrix_##F_SFX##_get_row_count(B)) { \
        goto MatrixProdDimErr; \
    } \
 \
    size_t const row_count = spm_matrix_##F_SFX##_get_row_count(A); \
    size_t const col_count = spm_matrix_##F_SFX##_get_col_count(B); \
    SpmMatrix##T_SFX *C = spm_matrix_##F_SFX##_make(row_count, col_count); \
 \
    if (!C) { \
...
SpmErr matrix_prod_tn( \
    SpmMatrix##T_SFX const *A, \
    SpmMatrix##T_SFX const *B, \
    SpmMatrix##T_SFX *C \
) { \
    SpmMatrix##T_SFX *At = spm_matrix_##F_SFX##_clone(A); \
    if (!At) { \
...
SpmErr matrix_prod_nn( \
    ...
    size_t const row_count = spm_matrix_##F_SFX##_get_row_count(C); \
    size_t const col_count = spm_matrix_##F_SFX##_get_col_count(C); \
 \
 ...
 \
            for (size_t A_curr = A_start; A_curr < A_end; A_curr++) { \
                SpmMatrix##T_SFX##Entry const *A_entry, *B_entry; \
 \
                A_entry = spm_list_##F_SFX##_entry_atc(&A->entries, A_curr); \
 ...
 \
                ...
                value = SUM( \
                    value, \
                    PROD(A_entry->value, B_entry->value) \
                ); \
            } \
 \
            if (IS_ZERO(value)) { \
 ...
 \
            ...
            size_t const C_entry_count = spm_list_##F_SFX##_entry_get_count(&C->entries); \
...
SpmErr matrix_prod_nt( \
 ...
 \
                A_entry = spm_list_##F_SFX##_entry_atc(&A->entries, A_curr); \
                B_entry = spm_list_##F_SFX##_entry_atc(&B->entries, B_curr); \
 \
...
inline SpmErr matrix_value_insert( \
    ...
    SpmMatrix##T_SFX##Entry entry = {.value=value, .col_index=col, .row_index=row}; \
...
SpmErr matrix_value_remove( \
 ...
 \
 \
void matrix_toggle_transpose_bool(SpmMatrix##T_SFX *m) { \
 ...
 \
 \
void matrix_transpose_values(SpmMatrix##T_SFX *m) { \
    size_swap(&m->row_count, &m->col_count); \
    list_size_swap(&m->row_indexes, &m->col_indexes); \
    size_t const entry_count = spm_list_##F_SFX##_entry_get_count(&m->entries); \
    for (size_t i = 0; i < entry_count; i++) { \
        SpmMatrix##T_SFX##Entry *entry = spm_list_##F_SFX##_entry_at(&m->entries, i); \
 ...
 \
    SpmMatrix##T_SFX##Entry *entries = m->entries.items; \
    size_t *new_row_starts = m->row_indexes.items; \
    size_t *write_pointers = m->col_indexes.items; \
 ...
 \
    for (size_t orig_index = 0; orig_index < m->row_count; orig_index++) { \
        while (write_pointers[orig_index] < new_row_starts[orig_index + 1]) { \
            ...
            SpmMatrix##T_SFX##Entry *e = &entries[curr_idx]; \
 \
 ...
 \
 \
void matrix_rebuild_col_indexes(SpmMatrix##T_SFX *m) { \
    size_t entry_count = m->entries.count; \
    if (entry_count == 0) return; \
 \
    SpmMatrix##T_SFX##Entry *entries = m->entries.items; \
    size_t *col_indexes = m->col_indexes.items; \
    size_t col_count = m->col_count; \
 \
...
SpmErr entry_insert( \
    ...
    SpmErr err = spm_list_##F_SFX##_entry_insert_at(entries, entry_index, 1); \
    switch (err) { \
 ...
 \
    spm_list_##F_SFX##_entry_set(entries, entry_index, entry); \
 \
    return SPM_ERR__OK; \
}; \
 \
...
SpmMatrix##T_SFX##Entry * entry_get_at(SpmMatrix##T_SFX *m, size_t row, size_t col) { \
 ...
 \
    SpmMatrix##T_SFX##Entry * entry = spm_list_##F_SFX##_entry_at(&m->entries, entry_index); \
 \
 ...
 \
 \
inline SpmMatrix##T_SFX##Entry const * entry_getc_at( \
    SpmMatrix##T_SFX const *m, \
    size_t row, \
    size_t col \
) { \
    return entry_get_at((SpmMatrix##T_SFX *)m, row, col); \
} \
 \
 \
size_t entry_index_get_at(SpmMatrix##T_SFX const *m, size_t row, size_t col) { \
    if (row >= m->row_count || col >= m->col_count) { \
        return spm_list_##F_SFX##_entry_get_count(&m->entries); \
    } \
...
size_t entry_index_get_with_col( \
 ...
 \
 \
inline void entry_swap(SpmMatrix##T_SFX##Entry *a, SpmMatrix##T_SFX##Entry *b) { \
    SpmMatrix##T_SFX##Entry c = *a; \
    *a = *b; \
    *b = c; \
}; \
 \
...
inline void list_size_swap(SpmListSize *a, SpmListSize *b) { \
 ...
 \
 \
inline void list_entry_swap(SpmList##T_SFX##Entry *a, SpmList##T_SFX##Entry *b) { \
    SpmList##T_SFX##Entry c = *a; \
    *a = *b; \
    *b = c; \
}; \
 \
